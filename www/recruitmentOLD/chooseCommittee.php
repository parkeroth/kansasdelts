...
<?php
	...
	if(isset($_POST['submit']))
	{
		...
		{	
			...
			if( strpos($members[$i]['accountType'], "recruitComm") ){
				
				$str = substr_replace($members[$i]['accountType'], '', strpos($members[$i]['accountType'], '|recruitComm'), 8);
			...
			if( isset($_POST[$members[$i]['username']]) ){
				...
				$str = $members[$i]['accountType']."|recruitComm";
...
<h1>Choose Recruitment Committee</h1>

	<form id="chooseRecruitment" name="chooseRecruitment" method="post" action="<?php echo $_SERVER['../academics/PHP_SELF']; ?>" onSubmit="return Confirm();">
		
		<p>
        	Put a check next to any user who is on the recruitment commitee. This will give them access to the recruitment system.
		</p>
		<p>&nbsp;</p>
		<?php
