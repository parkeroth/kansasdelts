...
$moveFiles = move_uploaded_file($_FILES['Filedata']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/pics/upload/files/' . $_FILES['Filedata']['name']);

if(!$moveFiles) 
{
	$error = 'Error moving uploaded files<br />'.$_FILES['Filedata']['tmp_name'].'<br />'.$_FILES['Filedata']['name'];
}
